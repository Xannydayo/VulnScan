...
def check_internet():
    ...
    val = 1 if "0% packet loss" in open('rs_net').read() else 0
...
def vul_info(val):
    severity = {
        'c': bcolors.BG_CRIT_TXT + " critical " + bcolors.ENDC,
        'h': bcolors.BG_HIGH_TXT + " high " + bcolors.ENDC,
        'm': bcolors.BG_MED_TXT + " medium " + bcolors.ENDC,
        'l': bcolors.BG_LOW_TXT + " low " + bcolors.ENDC,
    }
    return severity.get(val, bcolors.BG_INFO_TXT + " info " + bcolors.ENDC)
...
def vul_remed_info(v1, v2, v3):
    print(f"{bcolors.BOLD}Vulnerability Threat Level{bcolors.ENDC}")
    print(f"\t{vul_info(v2)} {bcolors.WARNING}{tool_resp[v1][0]}{bcolors.ENDC}")
    print(f"{bcolors.BOLD}Vulnerability Definition{bcolors.ENDC}")
    print(f"\t{bcolors.BADFAIL}{tools_fix[v3-1][1]}{bcolors.ENDC}")
    print(f"{bcolors.BOLD}Vulnerability Remediation{bcolors.ENDC}")
    print(f"\t{bcolors.OKGREEN}{tools_fix[v3-1][2]}{bcolors.ENDC}")


# RapidScan Help Context
def helper():
    print(f"{bcolors.OKBLUE}Information:{bcolors.ENDC}")
    print("------------")
    ...
    print(f"{bcolors.OKBLUE}Interactive:{bcolors.ENDC}")
    print("------------")
    print("\tCtrl+C: Skips current test.")
    print("\tCtrl+Z: Quits RapidScan.")
    print(f"{bcolors.OKBLUE}Legends:{bcolors.ENDC}")
    print("--------")
    print(f"\t[{proc_high}]: Scan process may take longer times (not predictable).")
    print(f"\t[{proc_med}]: Scan process may take less than 10 minutes.")
    print(f"\t[{proc_low}]: Scan process may take less than a minute or two.")
    print(f"{bcolors.OKBLUE}Vulnerability Information:{bcolors.ENDC}")
    print("--------------------------")
    print(f"\t{vul_info('c')}: Requires immediate attention as it may lead to compromise or service unavailability.")
    print(f"\t{vul_info('h')}    : May not lead to an immediate compromise, but there are considerable chances for probability.")
    print(f"\t{vul_info('m')}  : Attacker may correlate multiple vulnerabilities of this type to launch a sophisticated attack.")
    print(f"\t{vul_info('l')}     : Not a serious issue, but it is recommended to tend to the finding.")
    print(f"\t{vul_info('i')}    : Not classified as a vulnerability, simply an useful informational alert to be considered.\n")
            ...
            while self.busy:
                if not self.disabled:
                    ...
                    if inc > random.uniform(0, terminal_size()):
                        print(end="\r")
                        bcolors.BG_SCAN_TXT_START = f'\x1b[6;30;{round(random.uniform(40, 47))}m'
                        inc = 0
                    sys.stdout.flush()
